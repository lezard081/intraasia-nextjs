...
export default function ContactForm({ defaultMessage }: { defaultMessage?: string }) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Optional: Re-validate email on submit just in case
    if (!validateEmail(form.email)) {
      setErrors((prev) => ({ ...prev, email: 'Please enter a valid email address.' }));
      return; // Stop submission if email is invalid
    }
    // Optional: Add other validation here (e.g., name and message not empty)
    ...
    // Create FormData object directly from the event target (the form)
    const formData = new FormData(event.currentTarget);

    // If you want to include the honeypot value from state, you can append it
    // formData.append('bot-field', botField); // Though it should already be picked up by name

    // You can inspect formData if needed for debugging
    ...
    try {
      const response = await fetch('/api/contact', {
        method: 'POST',
        body: formData, // No need to set Content-Type, fetch handles it with FormData
      });
      ...
      if (response.ok) {
        ...
        setSubmitted(false); // Stay on the form to allow retry
      }
    } catch (error) {
